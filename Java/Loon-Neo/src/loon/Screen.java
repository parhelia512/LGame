...
public abstract class Screen extends PlayerUtils implements SysInput, IArray, LRelease, SetXY, XY {
	...
	public Screen setSpriteListener(SpriteListener sprListerner) {
		if (_currentSprites == null) {
			return this;
		}
		_currentSprites.setSpriteListener(sprListerner);
		return this;
	}
	...
	/**
	 * 重载此函数,可以自定义渲染Screen的最下层图像(api调用上位于位于画面渲染顺序最前)
	 * 
	 * @param g
	 */
	...
	/**
	 * 重载此函数,可以自定义渲染Screen的最上层图像(api调用上位于位于画面渲染顺序最后)
	 * 
	 * @param g
	 */
